<template>
  <div class="container">
    ...
    <ul>
      ...
      <li>
        <p>Карточки:
          <a class="button" @click="$router.push('/card-create/' + getUserInfo.id_user)">
            Добавить карточку
          </a>
        </p>
      </li>

      <div v-for="(myCards, index) in getCardInfo" :key="myCards.id_card">
        <div class="card" style="margin-bottom: 20px">
          <div class="card-body" style="padding-top: 0px;">
            <div class="card-header">
              <h5><label>Учебное учреждение № {{ index + 1 }}</label>
                <button class="close" aria-label="Close" v-on:click="deleteCard(myCards.id_card)">
            ...
            <h5 id="name">
              <p class="card-title">
                ...
                <a v-bind:href="'/card-update/' + myCards.id_card"> Edit </a>
              </p>
            </h5>

            <div v-for="myInst in myCards.institutions" :key="myInst.id">
              <label style="margin-right: 15px;">{{ myInst.name }}</label>
              <a v-bind:href="'/inst-update/' + myCards.id_card + '/' + myInst.id"> Edit </a>
              <a href="#" v-on:click="deleteInst(myCards.id_card, myInst.name)"> Del </a>
            </div>
